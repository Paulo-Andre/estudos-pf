...
export default function AccessGate({ onAuthenticated, initialMode = "login", onBackToStorefront, selectedPlanPending = false }: AccessGateProps) {
  ...
  const [form, setForm] = useState({ name: "", username: "", email: "", cpf: "", identifier: "", password: "", otp: "", confirmation: "", resetPassword: "", resetConfirmation: "" });
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        try {
          await login.mutateAsync({ identifier: form.identifier, password: form.password, otp: form.otp } as any);
        } catch (error:any) {
          if (error?.data?.mfaRequired) { setMfaRequired(true); setMessage("Digite o código do seu aplicativo autenticador ou um código de recuperação."); return; }
          throw error;
        }
      }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e6f4ef,transparent_32rem),linear-gradient(180deg,#f7f9f7,#f1f5f2)] px-3 py-0 text-[#152d38] sm:grid sm:place-items-center sm:px-4 sm:py-8">
    <section className="grid w-full max-w-5xl overflow-hidden rounded-[1.6rem] border border-[#d6e3de] bg-white shadow-[0_30px_90px_-45px_rgba(16,51,63,.5)] md:grid-cols-[0.92fr_1.08fr]">
      <div className="relative min-w-0 overflow-hidden bg-[linear-gradient(145deg,#153743,#0e5a70_70%,#17695f)] p-6 text-white sm:p-12">
        ...
        <div className="mb-3 inline-flex rounded-full border border-white/15 bg-white/8 px-3 py-1.5 text-[9px] font-bold uppercase tracking-[.16em] text-[#a9e3d6]">Acesso individual e privado</div>
        <h1 className="font-display mt-3 break-words text-[clamp(2.25rem,9vw,3.35rem)] font-extrabold leading-[1.02] tracking-[-.035em]">Seu estudo, organizado e protegido.</h1>
      ...
      <div className="min-w-0 bg-white p-5 sm:p-10 lg:p-12">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          {mode === "register" && <><label className="block text-xs font-bold">Nome completo<Input value={form.name} onChange={event => update("name", event.target.value)} autoComplete="name" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">Nome de usuário<Input value={form.username} onChange={event => update("username", event.target.value.toLowerCase())} autoComplete="username" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">E-mail<Input type="email" value={form.email} onChange={event => update("email", event.target.value)} autoComplete="email" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">CPF<Input value={form.cpf} onChange={event => update("cpf", formatCpfInput(event.target.value))} inputMode="numeric" autoComplete="off" placeholder="000.000.000-00" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" aria-describedby="cpf-help" required /></label><p id="cpf-help" className="-mt-2 text-[11px] leading-4 text-[#6f7775]">Usamos somente números e confirmamos os dígitos verificadores.</p></>}
          {mode === "login" && <><label className="block text-xs font-bold">Usuário ou e-mail<Input value={form.identifier} onChange={event => update("identifier", event.target.value)} autoComplete="username" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">Senha<Input type="password" value={form.password} onChange={event => update("password", event.target.value)} autoComplete="current-password" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label>{mfaRequired && <label className="block text-xs font-bold">Código de autenticação<Input inputMode="numeric" autoComplete="one-time-code" value={form.otp} onChange={event => update("otp", event.target.value.replace(/\s/g,"").slice(0,32))} placeholder="6 dígitos ou código de recuperação" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label>}<button type="button" onClick={() => { setMode("forgot"); setMessage(null); setMfaRequired(false); }} className="-mt-1 text-xs font-bold text-[#0e5a70] hover:underline">Esqueci minha senha</button></>}
          {mode === "register" && <><label className="block text-xs font-bold">Senha<Input type="password" value={form.password} onChange={event => update("password", event.target.value)} autoComplete="new-password" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">Confirme a senha<Input type="password" value={form.confirmation} onChange={event => update("confirmation", event.target.value)} autoComplete="new-password" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label></>}
          {mode === "forgot" && <label className="block text-xs font-bold">E-mail cadastrado<Input type="email" value={form.email} onChange={event => update("email", event.target.value)} autoComplete="email" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label>}
          {mode === "reset" && <><p className="rounded-xl border border-[#b9d6cb] bg-[#edf8f4] px-3 py-2 text-xs leading-5 text-[#17644e]"><KeyRound className="mr-1 inline h-4 w-4" />Use uma senha de pelo menos 12 caracteres. Ao confirmar, seus acessos ativos serão encerrados por segurança.</p><label className="block text-xs font-bold">Nova senha<Input type="password" value={form.resetPassword} onChange={event => update("resetPassword", event.target.value)} autoComplete="new-password" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label><label className="block text-xs font-bold">Confirme a nova senha<Input type="password" value={form.resetConfirmation} onChange={event => update("resetConfirmation", event.target.value)} autoComplete="new-password" className="mt-1.5 h-12 rounded-xl border-[#cddbd6] bg-[#fbfdfc] focus-visible:ring-[#8ad2c3]" required /></label></>}
