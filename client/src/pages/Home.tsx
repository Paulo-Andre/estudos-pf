...
import { officialItemsByDiscipline } from "@/data/pfCuratedVideos";
...
function StudyArea({ state, onOpen }: { state: StudyState; onOpen: (module: StudyModule) => void }) {
  const byDiscipline = studyModules.reduce<Record<string, StudyModule[]>>((groups, module) => { (groups[module.discipline] ??= []).push(module); return groups; }, {});
  return <div className="space-y-7">
    <section className="flex flex-col justify-between gap-4 border-b border-[#d7cfc1] pb-6 sm:flex-row sm:items-end">
      <div><p className="eyebrow">MATRIZ OFICIAL INTEGRAL</p><h2 className="font-display mt-2 text-3xl font-extrabold">Estude o edital sem lacunas.</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#62727a]">Os dossiês são agrupamentos pedagógicos, não redução do edital: abra cada um para ver o checklist literal dos subitens. A faixa de cada disciplina mostra a quantidade de itens oficiais conferidos.</p></div>
      <div className="rounded-xl border border-[#d5cdbd] bg-[#fffdf8] px-4 py-3 text-sm"><span className="font-bold text-[#0e5a70]">{state.completedModules.length}</span> de {studyModules.length} dossiês concluídos</div>
    </section>
    {Object.entries(byDiscipline).map(([discipline, modules]) => {
      const officialCount = officialItemsByDiscipline[discipline] ?? modules.reduce((total, module) => total + module.checklist.length, 0);
      return <section key={discipline}>
        <div className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-1"><span className="font-display text-sm font-bold text-[#0e5a70]">{discipline}</span><span className="h-px min-w-8 flex-1 bg-[#d7cfc1]" /><span className="text-[10px] font-bold tracking-wider text-[#8a7561]">BLOCO {modules[0].block} · {modules.length} DOSSIÊS · {officialCount} ITENS OFICIAIS</span></div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{modules.map((module) => {
          ...
          return <button key={module.id} onClick={() => onOpen(module)} className="group text-left shell-card relative overflow-hidden p-5 transition hover:-translate-y-0.5 hover:shadow-[0_20px_35px_-28px_rgba(14,90,112,.8)]">
            ...
            <div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1 text-[10px] font-bold tracking-wide text-[#76848a]"><Clock3 className="h-3.5 w-3.5" />{module.estimatedMinutes} MIN</span><span className="text-[10px] font-bold text-[#0e5a70]">{module.checklist.length} SUBITENS MAPEADOS</span></div>
            <div className="mt-4 flex items-center gap-1 text-xs font-bold text-[#0e5a70]">Abrir dossiê e conferir <ChevronRight className="h-4 w-4 transition group-hover:translate-x-1" /></div>
          </button>;
        })}</div>
      </section>;
    })}
