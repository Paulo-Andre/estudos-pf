...
function Dashboard({ state, level, totalAnswers, overallScore, streak, studiedPercent, focus, historyChart, onStudy, onSimulate }: { state: StudyState; level: ReturnType<typeof levelFromXp>; totalAnswers: number; overallScore: number; streak: number; studiedPercent: number; focus: { label: string; detail: string }; historyChart: { label: string; score: number }[]; onStudy: () => void; onSimulate: () => void }) {
  ...
  return <div className="space-y-7">
    <section className="relative overflow-hidden rounded-md border border-[#0c3442] bg-[#183542] px-6 py-8 text-white sm:px-9 sm:py-11" style={{ backgroundImage: `linear-gradient(90deg, rgba(21,45,56,.98) 0%, rgba(21,45,56,.9) 45%, rgba(21,45,56,.44) 100%), url(${imageUrls.desk})`, backgroundSize: "cover", backgroundPosition: "center" }}>
      <div className="absolute left-0 top-0 h-full w-1 bg-[#82cfbf]" /><div className="relative max-w-2xl"><div className="mb-4 flex items-center gap-3"><p className="text-[10px] font-bold tracking-[0.22em] text-[#9edbcf]">OPERAÇÃO DE HOJE · 01</p><span className="h-px w-12 bg-[#82cfbf]/60" /><span className="text-[9px] font-bold tracking-[0.14em] text-[#cceae2]">DOSSIÊ / ABERTO</span></div><h2 className="font-display text-3xl font-extrabold leading-tight sm:text-5xl">Preparação é evidência acumulada.</h2><p className="mt-4 max-w-xl text-sm leading-6 text-[#d4e0df] sm:text-base">Organize o estudo pelo edital, registre cada avanço e descubra com precisão onde concentrar a próxima missão.</p><div className="mt-7 flex flex-wrap gap-3"><button className="action-button bg-[#8ad2c3] text-[#17343e] hover:bg-[#b3e7db]" onClick={onStudy}><BookOpen className="h-4 w-4" />Continuar estudos</button><button className="ghost-button border-white/30 bg-white/10 text-white hover:bg-white/15 hover:text-white" onClick={onSimulate}><Play className="h-4 w-4" />Iniciar simulado</button></div></div>
      <div className="absolute right-5 top-5 hidden border border-white/20 bg-[#0f2a34]/80 px-3 py-2 text-[10px] font-bold tracking-wider text-[#cceae2] sm:block">META / {studyModules.length} MÓDULOS</div>
    </section>
...
function StudyArea({ state, onOpen }: { state: StudyState; onOpen: (module: StudyModule) => void }) { const byDiscipline = studyModules.reduce<Record<string, StudyModule[]>>((groups, module) => { (groups[module.discipline] ??= []).push(module); return groups; }, {}); return <div className="space-y-7"><section className="flex flex-col justify-between gap-4 border-b border-[#d7cfc1] pb-6 sm:flex-row sm:items-end"><div><p className="eyebrow">MATRIZ OFICIAL INTEGRAL</p><h2 className="font-display mt-2 text-3xl font-extrabold">Estude o edital sem lacunas.</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#62727a]">Cada dossiê traduz todos os subitens do programa em uma rota curta: síntese, conceitos, pontos de cobrança, exemplo, mnemônico e checklist literal de cobertura.</p></div><div className="rounded-xl border border-[#d5cdbd] bg-[#fffdf8] px-4 py-3 text-sm"><span className="font-bold text-[#0e5a70]">{state.completedModules.length}</span> de {studyModules.length} módulos concluídos</div></section>{Object.entries(byDiscipline).map(([discipline, modules]) => <section key={discipline}><div className="mb-3 flex items-center gap-3"><span className="font-display text-sm font-bold text-[#0e5a70]">{discipline}</span><span className="h-px flex-1 bg-[#d7cfc1]" /><span className="text-[10px] font-bold tracking-wider text-[#8a7561]">BLOCO {modules[0].block} · {modules.length} MÓDULOS</span></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{modules.map((module) => { const done = state.completedModules.includes(module.id); return <button key={module.id} onClick={() => onOpen(module)} className="group text-left shell-card relative overflow-hidden p-5 transition hover:-translate-y-0.5 hover:shadow-[0_20px_35px_-28px_rgba(14,90,112,.8)]"><div className="mb-4 flex items-start justify-between"><span className="rounded-lg bg-[#e8f0ee] px-2 py-1 text-[10px] font-bold tracking-wider text-[#0e5a70]">{module.code}</span>{done ? <span className="flex items-center gap-1 text-[10px] font-bold text-[#16806b]"><Check className="h-3.5 w-3.5" />REGISTRADO</span> : <span className="text-[10px] font-bold text-[#ad8a58]">+20 XP</span>}</div><h3 className="font-display text-base font-bold">{module.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-5 text-[#67767d]">{module.summary}</p><div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1 text-[10px] font-bold tracking-wide text-[#76848a]"><Clock3 className="h-3.5 w-3.5" />{module.estimatedMinutes} MIN</span><span className="text-[10px] font-bold text-[#0e5a70]">{module.checklist.length} TÓPICOS</span></div><div className="mt-4 flex items-center gap-1 text-xs font-bold text-[#0e5a70]">Abrir dossiê <ChevronRight className="h-4 w-4 transition group-hover:translate-x-1" /></div></button>; })}</div></section>)}</div>; }
