...
function StudyWorkspace({ user, logout, initialView, initialCommercePlanId, onCommercePlanConsumed }: { user: { name: string; username: string | null; email: string | null; role: "user" | "admin" }; logout: () => Promise<void>; initialView: View; initialCommercePlanId?: string | null; onCommercePlanConsumed: () => void }) {
  ...
  function registerAnswer(question: StudyQuestion, correct: boolean) {
    ...
    answerMutation.mutate({ questionId: question.id, correct }, { onSuccess: serverState => { setState(serverState as StudyState); void personalReviewsQuery.refetch(); void learningPlanQuery.refetch(); } });
  }
...
function WeeklyStudyPlanner({ progressItems, roadmapItems, learningPlan, onOpenScheduledContent, onSaveRoadmap, onRemoveRoadmap, saving }: { progressItems: StudyProgressItem[]; roadmapItems: RoadmapItem[]; learningPlan: LearningPlan | null; onOpenScheduledContent: (contentId: number) => void; onSaveRoadmap: (input: { disciplineId: number; weekday: number; isActive: boolean }) => void; onRemoveRoadmap: (id: number) => void; saving: boolean }) {
  ...
  const weakName=learningPlan?.weaknesses?.[0]?.discipline??"";
  const weakLower=weakName.toLocaleLowerCase("pt-BR");
  const recommendedDiscipline=disciplines.find(item=>{const name=item.name.toLocaleLowerCase("pt-BR");return Boolean(weakLower)&&(name.includes(weakLower)||weakLower.includes(name));});
    ...
    {learningPlan&&<section className="soft-panel p-4 sm:p-5">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-2xl bg-white p-4">
          ...
          <p className="mt-2 text-sm font-bold text-[#31515a]">Evite repetir a mesma disciplina em blocos consecutivos quando puder.</p>
        </div>
      </div>
    </section>}
...
function StudyArea({ state, modules, contestName, contentByModuleId, onOpen }: { state: StudyState; modules: StudyModule[]; contestName: string; contentByModuleId: Map<string, StudyProgressItem>; onOpen: (module: StudyModule) => void }) {
  ...
  return <div className="space-y-6"><section className="section-heading"><div><p className="eyebrow">BIBLIOTECA · {contestName}</p><h2 className="font-display mt-1 text-2xl font-extrabold tracking-[-.02em] text-[#173d4a] sm:text-3xl">Sua biblioteca de estudo</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#64777a]">Aulas organizadas por disciplina, com teoria, exemplos, prática ativa e anotações privadas.</p></div><div className="rounded-2xl border border-[#d8e6e1] bg-white px-4 py-3 shadow-sm"><p className="text-[9px] font-bold uppercase tracking-[.13em] text-[#748689]">Progresso</p><p className="font-display mt-1 text-lg font-extrabold text-[#0e5a70]">{completed}/{modules.length}</p></div></section>{Object.entries(byDiscipline).map(([discipline,items])=>{const done=items.filter(module=>state.completedModules.includes(module.id)).length;return <section key={discipline} className="shell-card overflow-hidden"><div className="flex flex-col gap-2 border-b border-[#e6edea] bg-[#f8fbfa] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div><h3 className="font-display font-extrabold text-[#23434e]">{discipline}</h3><p className="mt-1 text-[10px] font-bold uppercase tracking-[.12em] text-[#829094]">Bloco {items[0].block} · {items.length} aulas</p></div><div className="flex items-center gap-2"><div className="h-2 w-28 overflow-hidden rounded-full bg-[#e3ece8]"><div className="h-full rounded-full bg-[#16806b]" style={{width:`${items.length?Math.round(done/items.length*100):0}%`}}/></div><span className="text-[10px] font-bold text-[#52716f]">{done}/{items.length}</span></div></div><div className="grid gap-px bg-[#e6edea] md:grid-cols-2 xl:grid-cols-3">{items.map(module=>{const done=state.completedModules.includes(module.id);const notice=contentByModuleId.get(module.id)?.notice;return <button key={module.id} onClick={()=>onOpen(module)} className="group relative min-h-[210px] bg-white p-5 text-left transition hover:z-10 hover:bg-[#f8fcfa]"><div className="flex items-start justify-between gap-3"><div className="flex flex-wrap gap-2"><span className="rounded-lg bg-[#edf5f2] px-2 py-1 text-[9px] font-bold uppercase tracking-[.12em] text-[#0e5a70]">{module.code}</span>{notice&&<span className={`rounded-full px-2 py-1 text-[9px] font-bold ${notice.kind==="new"?"bg-[#eaf8f3] text-[#176a5a]":"bg-[#fff4df] text-[#91631b]"}`}>{notice.label}</span>}</div>{done?<span className="grid h-7 w-7 place-items-center rounded-full bg-[#e7f5ef] text-[#16806b]"><Check className="h-4 w-4"/></span>:<span className="text-[10px] font-bold text-[#a88b5a]">+20 XP</span>}</div><h4 className="font-display mt-4 text-base font-extrabold leading-6 text-[#23434e]">{module.title}</h4><p className="mt-2 line-clamp-2 text-sm leading-5 text-[#687a7d]">{module.summary}</p><div className="mt-5 flex items-center justify-between border-t border-[#edf1ef] pt-4"><span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#7b898c]"><Clock3 className="h-3.5 w-3.5"/>{module.estimatedMinutes} min</span><span className="inline-flex items-center gap-1 text-xs font-bold text-[#0e5a70]">Abrir <ChevronRight className="h-4 w-4 transition group-hover:translate-x-1"/></span></div></button>})}</div></section>})}</div>;
}
...
function SimulationResult({ result, onAgain, onClose, onReflectionSaved }: { result: SimulationRecord; onAgain: () => void; onClose: () => void; onReflectionSaved: () => void }) {
  const score=percentage(result.correct,result.total);
  const weak=Object.entries(result.byDiscipline).sort((a,b)=>percentage(a[1].correct,a[1].total)-percentage(b[1].correct,b[1].total))[0];
  const [confidence,setConfidence]=useState(result.reflection?.confidence ?? 3);
  const [primaryCause,setPrimaryCause]=useState<NonNullable<SimulationRecord["reflection"]>["primaryCause"]>(result.reflection?.primaryCause ?? "knowledge");
  const [nextAction,setNextAction]=useState<NonNullable<SimulationRecord["reflection"]>["nextAction"]>(result.reflection?.nextAction ?? "review");
  const [note,setNote]=useState(result.reflection?.note ?? "");
  const [saved,setSaved]=useState(Boolean(result.reflection));
  const reflectionMutation=trpc.study.simulationReflection.save.useMutation({
    onSuccess:()=>{setSaved(true);onReflectionSaved();},
  });
  const causes=[
    {id:"knowledge",label:"Conteúdo",detail:"Eu não dominava o conceito."},
    {id:"interpretation",label:"Interpretação",detail:"Entendi o enunciado de forma errada."},
    {id:"attention",label:"Atenção",detail:"Errei por distração ou pressa."},
    {id:"time",label:"Tempo",detail:"A gestão do tempo prejudicou."},
    {id:"strategy",label:"Estratégia",detail:"A ordem ou abordagem não ajudou."},
  ] as const;
  const actions=[
    {id:"review",label:"Revisar erros"},
    {id:"practice",label:"Praticar questões"},
    {id:"content",label:"Retomar conteúdo"},
    {id:"time_strategy",label:"Treinar tempo"},
    {id:"simulate",label:"Novo simulado"},
  ] as const;
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="shell-card p-5 sm:p-6">
      <div className="section-heading"><div><p className="eyebrow">REFLEXÃO DE 1 MINUTO</p><h3 className="font-display mt-1 text-xl font-extrabold text-[#173d4a]">Descubra por que você errou.</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#64777a]">Registrar a causa ajuda a não repetir o mesmo padrão. Não tente justificar cada questão: identifique o padrão dominante deste simulado.</p></div><Brain className="hidden h-6 w-6 text-[#0e5a70] sm:block"/></div>
      <div className="mt-5">
        <p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#60777a]">Quão confiante você estava antes de ver o resultado?</p>
        <div className="mt-2 grid grid-cols-5 gap-2">{[1,2,3,4,5].map(value=><button key={value} type="button" onClick={()=>{setConfidence(value);setSaved(false);}} className={`min-h-11 rounded-xl border text-sm font-extrabold transition ${confidence===value?"border-[#0e5a70] bg-[#eaf5f2] text-[#0e5a70]":"border-[#dce6e1] bg-white text-[#60777a]"}`}>{value}</button>)}</div>
        <div className="mt-1 flex justify-between text-[9px] font-semibold text-[#8a9698]"><span>Baixa</span><span>Alta</span></div>
      </div>
      <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#60777a]">Principal causa dos erros</p><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{causes.map(cause=><button key={cause.id} type="button" onClick={()=>{setPrimaryCause(cause.id);setSaved(false);}} className={`rounded-xl border p-3 text-left transition ${primaryCause===cause.id?"border-[#9bcfc2] bg-[#edf8f4]":"border-[#dfe7e4] bg-white"}`}><span className="block text-xs font-extrabold text-[#294a53]">{cause.label}</span><span className="mt-1 block text-[10px] leading-4 text-[#6f7f82]">{cause.detail}</span></button>)}</div></div>
      <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#60777a]">Próxima ação concreta</p><div className="mt-2 flex flex-wrap gap-2">{actions.map(action=><button key={action.id} type="button" onClick={()=>{setNextAction(action.id);setSaved(false);}} className={`rounded-xl border px-3 py-2 text-xs font-bold transition ${nextAction===action.id?"border-[#0e5a70] bg-[#eaf5f2] text-[#0e5a70]":"border-[#dce6e1] bg-white text-[#60777a]"}`}>{action.label}</button>)}</div></div>
      <label className="mt-5 block text-[10px] font-bold uppercase tracking-[.14em] text-[#60777a]">Nota opcional<textarea value={note} onChange={event=>{setNote(event.target.value.slice(0,600));setSaved(false);}} rows={3} placeholder="Ex.: errei exceções e li rápido demais nas questões longas." className="mt-2 w-full resize-y rounded-xl border border-[#cfddd8] bg-white p-3 text-sm font-medium normal-case tracking-normal text-[#294a53] outline-none focus:border-[#0e5a70] focus:ring-2 focus:ring-[#8ad2c3]/40"/></label>
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><p className="text-[10px] text-[#718083]">{saved?"Reflexão salva no seu histórico.":"Salve para comparar seus padrões nos próximos simulados."}</p><button type="button" disabled={reflectionMutation.isPending} onClick={()=>reflectionMutation.mutate({simulationId:result.id,confidence,primaryCause,nextAction,note})} className="action-button disabled:opacity-60">{reflectionMutation.isPending?"Salvando...":saved?"Atualizar reflexão":"Salvar reflexão"}</button></div>
      {reflectionMutation.error&&<p className="mt-2 text-xs font-semibold text-[#99462e]">{reflectionMutation.error.message}</p>}
    </section>
    <section className="shell-card p-6"><p className="eyebrow">PRÓXIMA AÇÃO</p><h3 className="font-display mt-2 text-xl font-bold">{weak?`Revise ${weak[0]}`:"Mantenha o ritmo"}</h3><p className="mt-2 text-sm text-[#64757e]">{weak?`Seu aproveitamento neste eixo foi de ${percentage(weak[1].correct,weak[1].total)}%. Seus erros já foram enviados para a revisão espaçada. Faça recuperação ativa deles antes de tentar outro simulado.`:"O resultado ficará registrado no seu histórico."}</p><div className="mt-6 flex flex-wrap gap-3"><button onClick={onAgain} className="action-button"><RotateCcw className="h-4 w-4"/>Nova tentativa</button><button onClick={onClose} className="ghost-button">Ver histórico</button></div></section>
  </div>;
}
...
function HistoryArea({ state }: { state: StudyState }) {
  const sims=[...state.simulations].reverse();
  const scores=sims.map(sim=>percentage(sim.correct,sim.total));
  ...
  const trend=recent.length>=2&&previous.length>=2?avg(recent)-avg(previous):null;
  const reflected=sims.filter(sim=>sim.reflection).length;
  const causeLabels:Record<string,string>={knowledge:"Conteúdo",attention:"Atenção",time:"Tempo",interpretation:"Interpretação",strategy:"Estratégia"};
  const actionLabels:Record<string,string>={review:"Revisar erros",practice:"Praticar questões",content:"Retomar conteúdo",time_strategy:"Treinar tempo",simulate:"Novo simulado"};
  return <div className="space-y-5">
    <section className="section-heading"><div><p className="eyebrow">HISTÓRICO</p><h2 className="font-display mt-1 text-2xl font-extrabold tracking-[-.02em] text-[#173d4a] sm:text-3xl">Resultados que viram aprendizado</h2><p className="mt-2 text-sm leading-6 text-[#64777a]">Use o histórico para reconhecer padrões: desempenho, tempo e principalmente por que os erros aconteceram.</p></div><History className="hidden h-7 w-7 text-[#0e5a70] sm:block"/></section>
    {sims.length?<><section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric icon={History} label="Simulados" value={String(sims.length)} detail="registros acumulados" tone="teal"/><Metric icon={Gauge} label="Média recente" value={`${avg(recent)}%`} detail="últimos até 3 simulados" tone="blue"/><Metric icon={Target} label="Tendência" value={trend===null?"—":`${trend>0?"+":""}${trend} p.p.`} detail={trend===null?"mais dados necessários":trend>2?"evolução recente":trend<-2?"atenção à queda":"desempenho estável"} tone="amber"/><Metric icon={Brain} label="Debriefings" value={`${reflected}/${sims.length}`} detail="reflexões registradas" tone="orange"/></section>
      <div className="grid gap-3">{sims.map(sim=>{const score=percentage(sim.correct,sim.total);const reflection=sim.reflection;return <article key={sim.id} className="shell-card p-4 sm:p-5"><div className="grid gap-4 sm:grid-cols-[auto_1fr_auto] sm:items-start"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#edf6f3] text-center"><span className="font-display text-lg font-extrabold text-[#0e5a70]">{score}%</span></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display font-extrabold text-[#24434d]">Simulado de {sim.total} itens</h3><span className="rounded-full bg-[#f2f6f4] px-2 py-1 text-[9px] font-bold text-[#6e7e81]">{new Date(sim.date).toLocaleDateString("pt-BR")}</span></div><p className="mt-1 text-xs text-[#718083]">{sim.correct} acertos · {sim.total-sim.correct} erros · {formatTime(sim.elapsedSeconds)}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e6eeeb]"><div className={`h-full rounded-full ${score>=80?"bg-[#16806b]":score>=60?"bg-[#c38a2e]":"bg-[#c5663e]"}`} style={{width:`${score}%`}}/></div>{reflection?<div className="mt-3 rounded-xl border border-[#dce8e4] bg-[#f8fbfa] p-3"><div className="flex flex-wrap gap-2 text-[9px] font-bold uppercase tracking-[.1em]"><span className="rounded-full bg-[#eaf5f2] px-2 py-1 text-[#0e5a70]">Confiança {reflection.confidence}/5</span><span className="rounded-full bg-[#fff5e7] px-2 py-1 text-[#91631b]">{causeLabels[reflection.primaryCause]??reflection.primaryCause}</span><span className="rounded-full bg-[#eef4f7] px-2 py-1 text-[#315f70]">{actionLabels[reflection.nextAction]??reflection.nextAction}</span></div>{reflection.note&&<p className="mt-2 text-xs leading-5 text-[#60777a]">“{reflection.note}”</p>}</div>:<p className="mt-3 text-[10px] font-semibold text-[#8a9698]">Sem reflexão registrada neste simulado.</p>}</div><div className="text-right"><p className="text-[9px] font-bold uppercase tracking-[.12em] text-[#879396]">Status</p><p className="mt-1 inline-flex items-center gap-1 text-xs font-bold text-[#16806b]"><Check className="h-3.5 w-3.5"/>Concluído</p></div></div></article>})}</div>
    </>:<section className="shell-card"><EmptyState icon={History} title="Seu histórico começa no primeiro simulado" text="Resultados, tempo, debriefing e padrões de aprendizagem aparecerão aqui."/></section>}
  </div>;
}
