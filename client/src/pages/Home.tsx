...
function StudyWorkspace({ user, logout, initialView, initialCommercePlanId, onCommercePlanConsumed }: { user: { name: string; username: string | null; email: string | null; role: "user" | "admin" }; logout: () => Promise<void>; initialView: View; initialCommercePlanId?: string | null; onCommercePlanConsumed: () => void }) {
  ...
  const studyBundleQuery = trpc.study.bundle.useQuery({ courseId: effectiveContestId }, { enabled: canUseActiveCourse, refetchOnWindowFocus: false });
  ...
  const centralQuestionsQuery = trpc.study.questions.list.useQuery({ courseId: effectiveContestId }, { enabled: canUseActiveCourse, refetchOnWindowFocus: false });
  ...
  const performanceQuestions = useMemo<StudyQuestion[]>(() => (centralQuestionsQuery.data?.questions ?? []).filter(question => question.questionType === "certo_errado" && typeof question.answer === "boolean").map(question => ({
