    ...
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex">
      ...
      <main className="min-h-screen min-w-0 flex-1">
        ...
        <div className="mx-auto max-w-[1540px] p-4 sm:p-7 lg:p-10">{simulation ? <SimulationScreen simulation={simulation} onAnswer={submitSimulationAnswer} onExit={() => setSimulation(null)} /> : simulationResult ? <SimulationResult result={simulationResult} onAgain={() => startSimulation(simulationResult.total)} onClose={() => { setSimulationResult(null); setView("Histórico"); }} /> : <>
          {view === "Painel" && <Dashboard state={state} level={level} totalAnswers={totalAnswers} overallScore={overallScore} streak={streak} studiedPercent={studiedPercent} focus={focus} historyChart={historyChart} onStudy={() => setView("Conteúdo")} onSimulate={() => setView("Simulados")} />}
          {view === "Conteúdo" && <StudyArea state={state} onOpen={setOpenedModule} />}
...
function Dashboard({ state, level, totalAnswers, overallScore, streak, studiedPercent, focus, historyChart, onStudy, onSimulate }: { state: StudyState; level: ReturnType<typeof levelFromXp>; totalAnswers: number; overallScore: number; streak: number; studiedPercent: number; focus: { label: string; detail: string }; historyChart: { label: string; score: number }[]; onStudy: () => void; onSimulate: () => void }) {
  ...
  const contentModules = studyModules.filter((module) => apostilaByModule[module.id]);
  const contentSectionCount = contentModules.reduce((total, module) => total + (apostilaByModule[module.id]?.secoes.length ?? 0), 0);
  return <div className="space-y-7">
    <section className="relative overflow-hidden rounded-md border border-[#0c3442] bg-[#183542] px-6 py-8 text-white sm:px-9 sm:py-11" style={{ backgroundImage: `linear-gradient(90deg, rgba(21,45,56,.98) 0%, rgba(21,45,56,.9) 45%, rgba(21,45,56,.44) 100%), url(${imageUrls.desk})`, backgroundSize: "cover", backgroundPosition: "center" }}>
      ...
      <div className="absolute right-5 top-5 hidden border border-white/20 bg-[#0f2a34]/80 px-3 py-2 text-[10px] font-bold tracking-wider text-[#cceae2] sm:block">BIBLIOTECA / {studyModules.length} AULAS</div>
    </section>
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Metric icon={Zap} label="XP ACUMULADO" value={state.xp.toString()} detail={`Nível ${level.index} · ${level.label}`} color="teal" /><Metric icon={Gauge} label="DOMÍNIO GERAL" value={`${overallScore}%`} detail={`${totalAnswers} questões respondidas`} color="blue" /><Metric icon={BookOpen} label="AULAS CONCLUÍDAS" value={`${studiedPercent}%`} detail={`${state.completedModules.length}/${studyModules.length} aulas de conteúdo`} color="amber" /><Metric icon={Flame} label="SEQUÊNCIA" value={`${streak} dia${streak === 1 ? "" : "s"}`} detail="Constância registrada" color="orange" /></section>
...
function StudyArea({ state, onOpen }: { state: StudyState; onOpen: (module: StudyModule) => void }) {
  ...
  return <div className="space-y-7">
    <section className="flex flex-col justify-between gap-4 border-b border-[#d7cfc1] pb-6 sm:flex-row sm:items-end">
      <div><p className="eyebrow">BIBLIOTECA DE CONTEÚDO</p><h2 className="font-display mt-2 text-3xl font-extrabold">Estude o conteúdo sem atalhos.</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#62727a]">Cada aula combina explicação autoral, conceitos-chave, exemplo resolvido, atenção de prova e prática ativa. O mapa do edital aparece como referência; a prioridade aqui é entender e recuperar o conteúdo.</p></div>
      <div className="rounded-xl border border-[#d5cdbd] bg-[#fffdf8] px-4 py-3 text-sm"><span className="font-bold text-[#0e5a70]">{state.completedModules.length}</span> de {studyModules.length} aulas concluídas</div>
    </section>
    {Object.entries(byDiscipline).map(([discipline, modules]) => {
      const contentCount = modules.reduce((total, module) => total + (apostilaByModule[module.id]?.secoes.length ?? module.concepts.length), 0);
      return <section key={discipline}>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{modules.map((module) => {
          ...
          return <button key={module.id} onClick={() => onOpen(module)} className="group text-left shell-card relative overflow-hidden p-5 transition hover:-translate-y-0.5 hover:shadow-[0_20px_35px_-28px_rgba(14,90,112,.8)]">
            ...
            <div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1 text-[10px] font-bold tracking-wide text-[#76848a]"><Clock3 className="h-3.5 w-3.5" />{module.estimatedMinutes} MIN</span><span className="text-[10px] font-bold text-[#0e5a70]">{apostilaByModule[module.id]?.secoes.length ?? module.concepts.length} NÚCLEOS DE CONTEÚDO</span></div>
