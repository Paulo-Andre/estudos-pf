    ...
    <div className="workspace-surface min-h-screen text-[#152d38] lg:flex" style={{ color: brand.textColor }}>
      ...
      <main className="min-h-screen min-w-0 flex-1">
        ...
        <div className="workspace-content mx-auto max-w-[1480px] p-3 sm:p-6 lg:p-8"><ContestSelector contestId={effectiveContestId} courses={permittedCourses} onChange={setContestId} />{view === "Painel" && user.role !== "admin" && <StudentAlerts />}{simulation ? (simulation.mode === "real_exam" ? <RealExamScreen simulation={simulation} onConfirm={submitSimulationAnswer} onExit={() => setSimulation(null)} /> : <SimulationScreen simulation={simulation} onAnswer={submitSimulationAnswer} onExit={() => setSimulation(null)} />) : simulationResult ? <SimulationResult result={simulationResult} onReflectionSaved={(reflection) => { const simulationId=simulationResult.id; setSimulationResult(current=>current?{...current,reflection}:current); setState(current=>({...current,simulations:current.simulations.map(item=>item.id===simulationId?{...item,reflection}:item)})); }} onFollowReflectionAction={(action) => { setSimulationResult(null); if(action==="review"||action==="practice") setView("Revisar"); else if(action==="content") setView("Conteúdo"); else setView("Simulados"); }} onAgain={() => { const first = simulationResult.telemetry?.questions?.[0]; startSimulation(simulationResult.total, simulationResult.mode === "domain" ? first?.discipline : undefined, simulationResult.mode === "domain" ? first?.subject : undefined, simulationResult.mode ?? "practice"); }} onClose={() => { setSimulationResult(null); setView("Histórico"); }} /> : <>
          ...
          {!tutorialCourse && view === "Inteligência" && <IntelligenceArea data={(learningIntelligenceQuery.data ?? null) as LearningIntelligence | null} loading={learningIntelligenceQuery.isLoading} onRefresh={() => void learningIntelligenceQuery.refetch()} onStartDomainProof={(topic: IntelligenceTopic) => startSimulation(Math.min(10, topic.questionCount), topic.discipline, topic.subject, "domain")} onStartRealExam={() => { if (persistentSimulationQuestions.length < 10) { setSimulationNotice("Publique ao menos 10 questões para liberar o Modo Prova Real."); setView("Simulados"); return; } startSimulation(Math.min(60, persistentSimulationQuestions.length), undefined, undefined, "real_exam"); }} />}
...
function Simulations({ onStart, state, learningPlan, notice, strictReviewMode, centralCount }: { onStart: (size: number, focusDiscipline?: string, focusSubject?: string, mode?: SimulationMode) => void; state: StudyState; learningPlan: LearningPlan | null; notice: string | null; strictReviewMode: boolean; centralCount: number }) {
  ...
  return <div className="space-y-5"><section className="relative overflow-hidden rounded-[1.7rem] border border-[#164e5e] bg-[#143c49] p-5 text-white shadow-[0_28px_70px_-48px_rgba(9,47,61,.85)] sm:p-8"><div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-[#8ad2c3]/10 blur-xl"/><div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#a7e4d7]">SALA DE PROVA</p><h2 className="font-display mt-2 text-3xl font-extrabold tracking-[-.025em] sm:text-4xl">Treine sob pressão, evolua com dados.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#d4e5e2]">Escolha o tamanho do simulado. A distribuição mantém a proporção oficial entre os blocos da prova.</p>{strictReviewMode&&<p className="mt-4 max-w-2xl rounded-xl border border-[#81b9b2]/50 bg-white/8 p-3 text-xs leading-5 text-[#e0f4ef]">Revisão obrigatória ativa: usando exclusivamente {centralCount} questão(ões) centrais aprovadas/publicadas.</p>}{notice&&<p role="status" className="mt-4 rounded-xl bg-[#fff7df] p-3 text-xs font-semibold text-[#6f5318]">{notice}</p>}</div><div className="grid grid-cols-3 gap-2">{modes.map(mode=><button key={mode.size} onClick={()=>onStart(mode.size)} className={`group min-w-20 rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:bg-[#9de0d1] hover:text-[#17343e] ${recommendedSize===mode.size?"border-[#9de0d1] bg-[#9de0d1]/15":"border-white/15 bg-white/8"}`}><div className="flex items-center justify-between gap-1"><p className="font-display text-2xl font-extrabold">{mode.size}</p>{recommendedSize===mode.size&&<span className="rounded-full bg-[#9de0d1] px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wider text-[#17343e]">recomendado</span>}</div><p className="mt-1 text-[9px] font-bold uppercase tracking-[.12em] opacity-75">{mode.label}</p><p className="mt-1 hidden text-[9px] leading-4 opacity-65 sm:block">{mode.detail}</p></button>)}</div></div></section>
    ...
    <section className="rounded-2xl border border-[#b8d6ce] bg-[#edf7f4] p-4 sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="eyebrow">MODO PROVA REAL · TELEMETRIA</p><h3 className="font-display mt-1 text-lg font-extrabold text-[#24434d]">Sem feedback até o fim.</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-[#64777a]">Registra tempo por item, trocas antes da confirmação, confiança e queda de desempenho entre as duas metades da prova.</p></div><button disabled={centralCount<10} onClick={()=>onStart(Math.min(60,centralCount),undefined,undefined,"real_exam")} className="action-button shrink-0 disabled:cursor-not-allowed disabled:opacity-45">Iniciar Prova Real</button></div>{centralCount<10&&<p className="mt-3 text-[10px] font-semibold text-[#8a6630]">Publique ao menos 10 questões para liberar este modo.</p>}</section>
...
const simulationReflectionCauseOptions = [
  { value: "knowledge", label: "Conteúdo", hint: "Faltou conhecimento ou lembrança do assunto." },
  { value: "attention", label: "Atenção", hint: "Você sabia, mas perdeu detalhe ou marcou sem conferir." },
  { value: "time", label: "Tempo", hint: "O ritmo ou a pressão do relógio prejudicou a execução." },
  { value: "interpretation", label: "Interpretação", hint: "A dificuldade foi entender comando, exceção ou enunciado." },
  { value: "strategy", label: "Estratégia", hint: "A ordem, escolha de itens ou forma de resolver precisa mudar." },
] as const;
const simulationReflectionActionOptions = [
  { value: "review", label: "Revisar erros" },
  { value: "practice", label: "Praticar questões" },
  { value: "content", label: "Retomar conteúdo" },
  { value: "time_strategy", label: "Treinar gestão de tempo" },
  { value: "simulate", label: "Novo simulado" },
] as const;
function simulationReflectionCauseLabel(value: string) { return simulationReflectionCauseOptions.find(item=>item.value===value)?.label ?? value; }
function simulationReflectionActionLabel(value: string) { return simulationReflectionActionOptions.find(item=>item.value===value)?.label ?? value; }

function SimulationResult({ result, onAgain, onClose, onReflectionSaved, onFollowReflectionAction }: { result: SimulationRecord; onAgain: () => void; onClose: () => void; onReflectionSaved: (reflection: NonNullable<SimulationRecord["reflection"]>) => void; onFollowReflectionAction: (action: NonNullable<SimulationRecord["reflection"]>["nextAction"]) => void }) {
  ...
  const initialReflection=result.reflection??null;
  const [reflectionConfidence,setReflectionConfidence]=useState(initialReflection?.confidence??3);
  const [primaryCause,setPrimaryCause]=useState<NonNullable<SimulationRecord["reflection"]>["primaryCause"]>(initialReflection?.primaryCause??"knowledge");
  const [nextAction,setNextAction]=useState<NonNullable<SimulationRecord["reflection"]>["nextAction"]>(initialReflection?.nextAction??(weak?"review":"simulate"));
  const [reflectionNote,setReflectionNote]=useState(initialReflection?.note??"");
  const [savedReflection,setSavedReflection]=useState<NonNullable<SimulationRecord["reflection"]>|null>(initialReflection);
  const reflectionMutation=trpc.study.simulationReflection.save.useMutation({
    onSuccess:(data:any)=>{
      const reflection=data as NonNullable<SimulationRecord["reflection"]>;
      setSavedReflection(reflection);
      onReflectionSaved(reflection);
    },
  });
  const saveReflection=()=>reflectionMutation.mutate({simulationId:result.id,confidence:reflectionConfidence,primaryCause,nextAction,note:reflectionNote});
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {result.mode==="real_exam"&&<section className="shell-card p-5 sm:p-6"><p className="eyebrow">TELEMETRIA DA PROVA REAL</p><h3 className="font-display mt-1 text-xl font-extrabold text-[#24434d]">Como você executou a prova</h3><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-[#f5faf8] p-3"><p className="text-[9px] font-bold uppercase text-[#748487]">Tempo médio/item</p><p className="mt-1 text-lg font-extrabold text-[#0e5a70]">{result.telemetry?.summary?.averageSeconds??"—"}s</p></div><div className="rounded-xl bg-[#f5faf8] p-3"><p className="text-[9px] font-bold uppercase text-[#748487]">Trocas</p><p className="mt-1 text-lg font-extrabold text-[#0e5a70]">{result.telemetry?.summary?.answerChanges??0}</p></div><div className="rounded-xl bg-[#f5faf8] p-3"><p className="text-[9px] font-bold uppercase text-[#748487]">Erros com alta confiança</p><p className="mt-1 text-lg font-extrabold text-[#0e5a70]">{result.telemetry?.summary?.highConfidenceErrors??0}</p></div><div className="rounded-xl bg-[#f5faf8] p-3"><p className="text-[9px] font-bold uppercase text-[#748487]">Queda 1ª → 2ª metade</p><p className="mt-1 text-lg font-extrabold text-[#0e5a70]">{result.telemetry?.summary?.performanceDrop??0} p.p.</p></div></div></section>}
    ...
    <section className="shell-card p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
        <div className="rounded-2xl border border-[#dbe7e3] bg-[#f8fbfa] p-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#677b7e]">Quão confiante você está no seu diagnóstico?</p><div className="mt-3 grid grid-cols-5 gap-2">{[1,2,3,4,5].map(value=><button key={value} type="button" onClick={()=>setReflectionConfidence(value)} aria-label={`Confiança ${value} de 5`} className={`min-h-10 rounded-lg border text-xs font-extrabold transition ${reflectionConfidence===value?"border-[#0e5a70] bg-[#e6f3ef] text-[#0e5a70]":"border-[#d9e4e0] bg-white text-[#6d7d80]"}`}>{value}</button>)}</div><p className="mt-2 text-[10px] leading-4 text-[#7a898c]">1 = ainda não entendi bem a causa · 5 = causa muito clara.</p></div>
        <div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#677b7e]">Principal causa</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{simulationReflectionCauseOptions.map(option=><button key={option.value} type="button" onClick={()=>setPrimaryCause(option.value)} className={`rounded-xl border p-3 text-left transition ${primaryCause===option.value?"border-[#0e5a70] bg-[#edf7f4]":"border-[#dfe7e3] bg-white hover:border-[#a8cfc5]"}`}><span className="block text-xs font-extrabold text-[#294a53]">{option.label}</span><span className="mt-1 block text-[10px] leading-4 text-[#718083]">{option.hint}</span></button>)}</div></div>
      </div>
      <div className="mt-5 grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><label><span className="text-[10px] font-bold uppercase tracking-[.12em] text-[#677b7e]">Próxima ação</span><select value={nextAction} onChange={event=>setNextAction(event.target.value as NonNullable<SimulationRecord["reflection"]>["nextAction"])} className="mt-2 h-11 w-full rounded-xl border border-[#d5e1dd] bg-white px-3 text-sm font-semibold text-[#31515a] outline-none focus:ring-2 focus:ring-[#82cfbf]">{simulationReflectionActionOptions.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label><span className="text-[10px] font-bold uppercase tracking-[.12em] text-[#677b7e]">Nota opcional</span><Textarea value={reflectionNote} onChange={event=>setReflectionNote(event.target.value.slice(0,600))} maxLength={600} placeholder="Ex.: errei exceções, acelerei no final, confundi dois conceitos..." className="mt-2 min-h-24 resize-y border-[#d5e1dd] bg-white text-sm leading-6"/><span className="mt-1 block text-right text-[9px] text-[#839093]">{reflectionNote.length}/600</span></label></div>
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div>{savedReflection?<p role="status" className="text-xs font-semibold text-[#17644e]">Reflexão salva · {simulationReflectionCauseLabel(savedReflection.primaryCause)} · próxima ação: {simulationReflectionActionLabel(savedReflection.nextAction)}.</p>:<p className="text-xs leading-5 text-[#718083]">A reflexão fica vinculada a este simulado e reaparece no histórico.</p>}{reflectionMutation.error&&<p role="alert" className="mt-1 text-xs font-semibold text-[#99462e]">{reflectionMutation.error.message}</p>}</div><div className="flex flex-col gap-2 sm:flex-row"><button type="button" disabled={reflectionMutation.isPending} onClick={saveReflection} className="ghost-button shrink-0 disabled:opacity-60">{reflectionMutation.isPending?"Salvando reflexão...":savedReflection?"Atualizar reflexão":"Salvar reflexão"}</button>{savedReflection&&<button type="button" onClick={()=>onFollowReflectionAction(savedReflection.nextAction)} className="action-button shrink-0">Seguir: {simulationReflectionActionLabel(savedReflection.nextAction)}<ChevronRight className="h-4 w-4"/></button>}</div></div>
    </section>
...
function HistoryArea({ state, learningPlan, onReview, onSimulate }: { state: StudyState; learningPlan: LearningPlan | null; onReview: () => void; onSimulate: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    {sims.length?<div className="grid gap-3">{sims.map(sim=>{const score=percentage(sim.correct,sim.total);return <article key={sim.id} className="shell-card p-4 sm:p-5"><div className="grid gap-4 sm:grid-cols-[auto_1fr_auto] sm:items-center"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#edf6f3] text-center"><span className="font-display text-lg font-extrabold text-[#0e5a70]">{score}%</span></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display font-extrabold text-[#24434d]">Simulado de {sim.total} itens</h3><span className="rounded-full bg-[#f2f6f4] px-2 py-1 text-[9px] font-bold text-[#6e7e81]">{new Date(sim.date).toLocaleDateString("pt-BR")}</span></div><p className="mt-1 text-xs text-[#718083]">{sim.correct} acertos · {sim.total-sim.correct} erros · {formatTime(sim.elapsedSeconds)}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e6eeeb]"><div className={`h-full rounded-full ${score>=80?"bg-[#16806b]":score>=60?"bg-[#c38a2e]":"bg-[#c5663e]"}`} style={{width:`${score}%`}}/></div><p className="mt-2 text-[10px] leading-4 text-[#829093]">{score>=80?"Boa estabilidade. Use os erros restantes como revisão espaçada.":score>=60?"Há domínio parcial. Corrija os erros antes de aumentar o tamanho do próximo simulado.":"Volte aos pontos fracos, faça recuperação ativa e repita um diagnóstico curto."}</p>{sim.reflection&&<div className="mt-3 rounded-xl border border-[#dce7e3] bg-[#f8fbfa] p-3"><p className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#55736f]">Reflexão registrada</p><p className="mt-1 text-xs font-semibold text-[#31515a]">{simulationReflectionCauseLabel(sim.reflection.primaryCause)} · confiança {sim.reflection.confidence}/5</p><p className="mt-1 text-[10px] text-[#718083]">Próxima ação: {simulationReflectionActionLabel(sim.reflection.nextAction)}</p>{sim.reflection.note&&<p className="mt-2 line-clamp-2 text-[10px] leading-4 text-[#718083]">{sim.reflection.note}</p>}</div>}</div><div className="text-right"><p className="text-[9px] font-bold uppercase tracking-[.12em] text-[#879396]">Status</p><p className="mt-1 inline-flex items-center gap-1 text-xs font-bold text-[#16806b]"><Check className="h-3.5 w-3.5"/>Concluído</p></div></div></article>})}</div>:<section className="shell-card"><EmptyState icon={History} title="Seu histórico começa no primeiro simulado" text="Resultados, tempo, tendência e diagnóstico aparecerão aqui."/></section>}
  </div>;
}
