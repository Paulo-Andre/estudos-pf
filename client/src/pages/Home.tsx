...
type LearningPlan = {
  ...
  recommendations: { type: string; title: string; detail: string; cta: string; contentId?: number }[];
  dueReviews: PersonalReviewItem[];
  weaknesses: { discipline: string; accuracy: number; correct: number; total: number }[];
  metrics: { readiness: number; progressPercent: number; reviewHealth: number; questionsThisWeek: number; questionGoal: number; studyDaysThisWeek: number; studyDayGoal: number };
};
...
function WeeklyStudyPlanner({ progressItems, roadmapItems, onOpenScheduledContent, onSaveRoadmap, onRemoveRoadmap, saving }: { progressItems: StudyProgressItem[]; roadmapItems: RoadmapItem[]; onOpenScheduledContent: (contentId: number) => void; onSaveRoadmap: (input: { disciplineId: number; weekday: number; isActive: boolean }) => void; onRemoveRoadmap: (id: number) => void; saving: boolean }) {
  ...
  return <div className="space-y-5"><section className="section-heading"><div><p className="eyebrow">PLANEJAMENTO</p><h2 className="font-display mt-1 text-2xl font-extrabold tracking-[-.02em] text-[#173d4a] sm:text-3xl">Sua semana de estudos</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#64777a]">Distribua disciplinas por dia. O objetivo é criar uma rotina simples de seguir, não uma agenda impossível de cumprir.</p></div><CalendarClock className="hidden h-7 w-7 text-[#0e5a70] sm:block"/></section>
...
function Simulations({ onStart, state, notice, strictReviewMode, centralCount }: { onStart: (size: number) => void; state: StudyState; notice: string | null; strictReviewMode: boolean; centralCount: number }) {
